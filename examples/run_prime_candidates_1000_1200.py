...
CANDIDATES_PATH = Path("examples/prime_candidates_1000_1200.jsonl")
TRUTH_PATH = Path("examples/prime_candidates_1000_1200_truth.jsonl")
OUTPUT_PATH = Path("examples/prime_candidates_1000_1200_scores.jsonl")
...
def compute_omnia_score(n: int) -> float:
    """
    Minimal integration point for the repo's documented OMNIA interface.

    This follows the public minimal interface idea:
    - import run_omnia_totale from omnia.engine
    - pass tokens and token_numbers in extra
    - read result.omega_total as the score

    The score is used here as a structural score for a numeric candidate.
    """
...
def summarize(records: List[Dict[str, Any]]) -> None:
    ...
    top_10 = sorted(records, key=lambda r: r["omnia_score"], reverse=True)[:10]
    top_20 = sorted(records, key=lambda r: r["omnia_score"], reverse=True)[:20]
    ...
    for record in sorted(records, key=lambda r: r["omnia_score"], reverse=True)[:15]:
        print(
