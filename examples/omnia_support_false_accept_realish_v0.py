...
DATA_PATH = Path("data/OMNIA_SUPPORT_FALSE_ACCEPT_REALISH_v0.jsonl")
...
def run_miniset(rows: list[dict]) -> tuple[list[dict], dict]:
    results = []
...
def print_results(results: list[dict], summary: dict) -> None:
    print("OMNIA SUPPORT FALSE ACCEPT REALISH V0")
    print("=" * 60)

    for r in results:
        print(f"ID: {r['id']} | GROUP: {r['group']} | TARGET: {r['target_label']}")
        print(f"TEXT: {r['text']}")
...
if __name__ == "__main__":
    ...
    results, summary = run_miniset(rows)
