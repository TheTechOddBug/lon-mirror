...
# ============================================================
# REPLICATION SUITE v0
# Goal:
# - test whether consistency can drop before accuracy
...
DATASET = [
    ...
    # Negative control: should stay stable
    # --------------------------------------------------------
    {
    ...
    # Can consistency degrade before accuracy?
    # --------------------------------------------------------
    {
    ...
    # Mixed family, should remain fairly stable
    # --------------------------------------------------------
    {
    ...
    # Negative control for false positives
    # --------------------------------------------------------
    {
...
def compute_deltas(rows):
    ...
    for row in rows:
        ...
        if prev_acc is None:
            ...
            current["delta_consistency"] = round(row["consistency"] - prev_cons, 4)

        prev_acc = row["accuracy"]
        prev_cons = row["consistency"]
...
def print_suite(payload):
    ...
    print("PRECOLLAPSE REPLICATION SUITE v0")
    print("=" * 88)
    print()
    ...
    for fam in families:
        ...
        for row in fam["rows"]:
            ...
            dc = "None" if row["delta_consistency"] is None else f"{row['delta_consistency']:+.4f}"

            print(
                f"  L{row['level']:>2} | "
                f"band={row['complexity_band']:<13} | "
                f"acc={row['accuracy']:.4f} | "
                f"cons={row['consistency']:.4f} | "
                f"class={row['consistency_class']:<9} | "
                f"dAcc={da} | "
                f"dCons={dc}"
            )
        ...
        if lead["early_drop_detected"]:
            first_drop_level = lead["first_consistency_drop"]["level"]
        else:
            first_drop_level = None

        if lead["first_accuracy_drop"] is not None:
            first_acc_drop_level = lead["first_accuracy_drop"]["level"]
        else:
            first_acc_drop_level = None
    ...
    print("- early_drop_detected means consistency decreased while accuracy remained 1.0")
    print("- positive lead means consistency deterioration appeared before the first accuracy loss")
...
if __name__ == "__main__":
    output_file = "examples/precollapse_replication_suite_results_v0.json"
